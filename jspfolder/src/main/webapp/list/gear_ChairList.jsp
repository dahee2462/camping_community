...
<%@ include file="/include/nav.jsp" %>
	<section>
<!-- 게시판페이지 이름 -->
		<h2>캠핑장비[의자/테이블] 게시판</h2>
		<div class="frms">
<!-- 게시글 정렬폼 -->
		<form name="frm1" action="gear_ChairList.jsp" method="get" id="frm1">
<!-- 게시글 정렬종류 -->
...
<!-- 게시글 검색폼 -->
		<form name="frm2" action="gear_ChairList.jsp" method="get" id="frm2">
			...
			<tr>
				...
				<td>
					...
					<a href="<%=request.getContextPath()%>
							/board/view.jsp?bno=<%=bno%>&blist=gear_Chair"><%=btitle %></a>
...
%>
	...
	<!-- 글쓰기 버튼: blist파라미터 넘기기 -->
		<button class="writeBtn" 
		 onclick="location.href='<%=request.getContextPath()%>/board/write.jsp?blist=gear_Chair'">글쓰기
		</button>
	</div>
<% 
	}
...
%>
		<span class="paging">
		<a href="gear_ChairList.jsp?nowPage=<%=pagingVO.getStartPage()-1%>
...
%>
				...
				<a href="gear_ChairList.jsp?nowPage=<%=i%>
					<%if(searchAlign!=null && !searchAlign.equals("")) out.print("&searchAlign="+searchAlign);
						if(searchType!=null && !searchAlign.equals("")) out.print("&searchType="+searchType);
						if(searchValue!=null && !searchAlign.equals("")) out.print("&searchValue="+searchValue);
						%>"><%=i %></a>
...
%>
		<span class="paging">
		<a href="gear_ChairList.jsp?nowPage=<%=pagingVO.getEndPage()+1%>
				<%if(searchAlign!=null && !searchAlign.equals("")) out.print("&searchAlign="+searchAlign);
						if(searchType!=null && !searchAlign.equals("")) out.print("&searchType="+searchType);
						if(searchValue!=null && !searchAlign.equals("")) out.print("&searchValue="+searchValue);
						%> " class="pluspage">다음</a>
